...
PanelWindow {
    ...
    MouseArea {
        anchors.fill: parent
        z: -9999
        visible: launcherOpen || settingsMenuOpen || (editorOpen && popupOpen)
        onClicked: {
            launcherOpen = false
    ...
    Process {
        id: brightnessProc
        command: ["sh", "-c", "D=$(ls /sys/class/backlight/ | head -1) && echo $(( $(cat /sys/class/backlight/$D/brightness) * 100 / $(cat /sys/class/backlight/$D/max_brightness) ))"]
        running: true
        stdout: SplitParser {
            onRead: data => {
    ...
    Process {
        id: audioSinkProcess
        command: ["sh", Qt.resolvedUrl("get_sinks.sh").toString().replace("file://", "")]
        running: true
        stdout: SplitParser {
            onRead: data => {
                try {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            var w = width;
            var h = height;
            var r = Settings.cornerRadius;

            ctx.beginPath();
            ctx.moveTo(0, 0); 
            ctx.lineTo(w, 0); 
            ctx.lineTo(w, h); 
            ctx.quadraticCurveTo(w - (r / 4), h - r, w - r, h - r);
            ctx.lineTo(r, h - r); 
            ctx.quadraticCurveTo(r / 4, h - r, 0, h);
            ctx.closePath();
    ...
    Rectangle {
        ...
        Behavior on scale { enabled: Settings.animationEnabled; NumberAnimation { duration: 200; easing.type: Easing.OutCubic } }
        ...
        MouseArea {
            ...
            onClicked: (mouse) => {
                if (mouse.button === Qt.RightButton) {
                    var newPet = Settings.mediaPet === "alien" ? "cat" : "alien";
                    Settings.setValue("widgets.media", "pet", newPet);
                    return;
                }
                if (mediaPlayer.overrideCat && mediaPlayer.activePlayer) {
                    mediaPlayer.overrideCat = false;
                } else if (mediaPlayer.activePlayer && !mediaPlayer.overrideCat) {
                    toggleMediaPopup()
                }
            }
        }
    ...
    PopupWindow {
        ...
        Rectangle {
            ...
            Behavior on opacity { enabled: Settings.animationEnabled; NumberAnimation { duration: 250; easing.type: Easing.OutCubic } }
            ...
            Row {
                ...
                anchors.left: albumArtContainer.right
                anchors.leftMargin: 12
                anchors.top: popupTrackArtist.bottom
                ...
                property real currentMediaVolume: 0.5
                
                Process {
                    id: mprisVolProc
                    function setMediaVol(ratio, busName) {
                        command = ["sh", Qt.resolvedUrl("set_media_vol.sh").toString().replace("file://", ""), ratio.toString(), busName];
                        running = true;
                    }
                }
                ...
                Timer {
                    ...
                    property string targetBus: ""
                    onTriggered: {
                        mprisVolProc.setMediaVol(targetRatio, targetBus);
                    }
                }
                property string currentBusName: mediaPlayer.activePlayer ? mediaPlayer.activePlayer.dbusName : ""
                
                onCurrentBusNameChanged: {
                    if (mprisVolumeRow.visible && currentBusName !== "") {
                        mediaVolProcess.running = false;
                ...
                Timer {
                    ...
                    interval: 10
                    onTriggered: {
                        mediaVolProcess.running = true;
                    }
                }
                
                Process {
                    id: mediaVolProcess
                    command: ["sh", Qt.resolvedUrl("media_vol.sh").toString().replace("file://", ""), mprisVolumeRow.currentBusName]
                    running: mprisVolumeRow.visible && mprisVolumeRow.currentBusName !== ""
                    
                    stdout: SplitParser {
                        onRead: data => {
                            var val = parseFloat(data.trim());
                            if (!isNaN(val) && !mprisVolDragArea.pressed) {
                                mprisVolumeRow.currentMediaVolume = val;
                            }
                        }
                    }
                }
                ...
                Text {
                    text: (mprisVolumeRow.currentMediaVolume === 0) ? "volume_off" : "volume_up"
                    font.family: materialFont.name
                    font.pixelSize: 16
                    color: Settings.textSecondary
                    anchors.verticalCenter: parent.verticalCenter
                ...
                Rectangle {
                    ...
                    Rectangle {
                        width: mprisVolumeRow.currentMediaVolume * parent.width
                        height: parent.height
                        radius: 2
                        color: Settings.accentColor
                        Behavior on width { enabled: Settings.animationEnabled; NumberAnimation { duration: 150; easing.type: Easing.OutCubic } }
                    }
                    ...
                    Rectangle {
                        ...
                        Behavior on width { enabled: Settings.animationEnabled; NumberAnimation { duration: 150 } }
                    }
                    ...
                    MouseArea {
                        ...
                        onPositionChanged: function(mouse) {
                            if (pressed) {
                                var ratio = Math.max(0, Math.min(1, mouse.x / mprisVolumeTrack.width));
                                mprisVolumeRow.currentMediaVolume = ratio;
                                throttleTimer.targetRatio = ratio;
                                throttleTimer.targetBus = mediaPlayer.activePlayer ? mediaPlayer.activePlayer.dbusName : "";
                                if (!throttleTimer.running) throttleTimer.start();
                            }
                        }
            ...
            Row {
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        Behavior on width { enabled: Settings.animationEnabled; NumberAnimation { duration: 200; easing.type: Easing.Linear } }
                    }
    ...
    Row {
        ...
        Repeater {
            ...
            delegate: Rectangle {
                ...
                Behavior on color { enabled: Settings.animationEnabled; ColorAnimation { duration: 150 } }
                Behavior on opacity { enabled: Settings.animationEnabled; NumberAnimation { duration: 150 } }
    ...
    SettingsMenuPopup {
        ...
        onCloseRequested: settingsMenuOpen = false
    }
    ...
    PopupWindow {
        ...
        Rectangle {
            ...
            Rectangle {
                ...
                width: 18
                radius: 9
                color: isRecording ? "#ffebeb" : (recordMouseArea.containsMouse ? Settings.hoverColor : "#ffffff")
                border.color: isRecording ? "#ff5555" : (recordMouseArea.containsMouse ? Settings.accentColor : Settings.borderColor)
                border.width: 1
                ...
                Image {
                    ...
                    height: 12
                    source: isRecording 
                        ? Qt.resolvedUrl("assets/watermelon_refresher.png") 
                        : Qt.resolvedUrl("assets/sprite_berry_blast.png")
                    fillMode: Image.PreserveAspectFit
                }
                ...
                MouseArea {
                    ...
                    onClicked: (mouse) => {
                        if (mouse.button === Qt.RightButton) {
                            Quickshell.execDetached(["sh", "-c", "xdg-open ~/Videos || xdg-open ~/videos"]);
                            return;
                        }
                        if (isRecording) {
                            ...
                            Quickshell.execDetached(["sh", "-c", "mkdir -p ~/videos && wf-recorder --audio=$(pw-metadata -n default 0 default.audio.sink | grep default.audio.sink | cut -d '\\\"' -f 4).monitor -f ~/videos/screenrecord_$(date +%y%m%d_%h%m%s).mp4"]);
                            isRecording = true;
                        }
                    }
                }
                ...
                Item {
                    ...
                    Process {
                        ...
                        function setBr(val) {
                            ...
                            command = ["sh", "-c", "D=$(ls /sys/class/backlight/ | head -1) && busctl call org.freedesktop.login1 /org/freedesktop/login1/session/auto org.freedesktop.login1.Session SetBrightness ssu backlight $D $(( " + pc + " * $(cat /sys/class/backlight/$D/max_brightness) / 100 ))"];
                            running = true;
                        }
                    }
            ...
            Row {
                ...
                Item {
                    id: customVolSlider
                    property real dragVol: Pipewire.defaultAudioSink ? Pipewire.defaultAudioSink.audio.volume : 0
                    ...
                    Rectangle {
                        ...
                        Rectangle {
                            width: (sliderMouseArea.pressed ? customVolSlider.dragVol : (Pipewire.defaultAudioSink ? Pipewire.defaultAudioSink.audio.volume : 0)) * parent.width
                            height: parent.height
                            color: Settings.accentColor
                            radius: 2
                        }
                    ...
                    Rectangle {
                        id: sliderHandle
                        x: (sliderMouseArea.pressed ? customVolSlider.dragVol : (Pipewire.defaultAudioSink ? Pipewire.defaultAudioSink.audio.volume : 0)) * (parent.width - width)
                        anchors.verticalCenter: parent.verticalCenter
                        width: 16
                        height: 16
                        radius: 8
            ...
            Item {
                ...
                clip: true
                Behavior on height { enabled: Settings.animationEnabled; NumberAnimation { duration: 250; easing.type: Easing.OutCubic } }
                Column {
                    anchors.fill: parent
                    anchors.margins: 12
                    spacing: 8
    ...
    Rectangle {
        ...
        Behavior on opacity { enabled: Settings.animationEnabled; NumberAnimation { duration: 200; easing.type: Easing.OutCubic } }
        Behavior on scale { enabled: Settings.animationEnabled; NumberAnimation { duration: 250; easing.type: Easing.OutBack } }
    ...
    PopupWindow {
        ...
        Rectangle {
            ...
            Behavior on opacity { enabled: Settings.animationEnabled; NumberAnimation { duration: 300; easing.type: Easing.OutCubic } }
            Behavior on scale { enabled: Settings.animationEnabled; NumberAnimation { duration: 300; easing.type: Easing.OutBack } }
            ...
            Row {
                ...
                Rectangle {
                    ...
                    Text {
                        anchors.centerIn: parent
                        text: "✕"
                        color: dismissMouseArea.containsMouse ? Settings.errorColor : Settings.textSecondary
                        font.pixelSize: 14
                        font.bold: true
                    }
    ...
    PopupWindow {
        ...
        color: "transparent"
        grabFocus: true
        onClosed: notificationsPopupOpen = false
    ...
    Item {
        ...
        Process {
            id: weatherProc
            command: ["curl", "-s", "https://api.open-meteo.com/v1/forecast?latitude=" + Settings.weatherLatitude + "&longitude=" + Settings.weatherLongitude + "&current_weather=true&temperature_unit=" + Settings.temperatureUnit]
            running: false
        ...
        Timer {
            interval: Settings.weatherInterval
            running: true
            repeat: true
            triggeredOnStart: true
            onTriggered: {
    ...
    Process {
        id: wlsunsetProc
        command: ["wlsunset", "-T", String(Settings.nightlightTempHigh), "-t", String(Settings.nightlightTempLow)]
